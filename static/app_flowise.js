document.addEventListener("DOMContentLoaded", function () {
  ...
  const flowiseIFrameURL = "http://127.0.0.1:5500/docfolio/index.htm";

  let currentDocuments = [];

  function showFlowiseIframe() {
    // Get the ID
    const queryString = window.location.search;
    const urlParams = new URLSearchParams(queryString);
    const id = urlParams.get("id") ?? "";

    // Set the iframe's src to flowiseFrameURL + id
    const flowiseIFrame = document.getElementById("flowiseIFrame");
    const url = flowiseIFrameURL + (id ? "?id=" + id : "");
    flowiseIFrame.src = url;
    console.log({ url }, { flowiseIFrameURL });
  }
  //showFlowiseIframe();
      ...
      .then((data) => {
        ...
        if (data.documents.length === 0) {
          ...
          data.documents.forEach((doc) => {
            ...
            const filename = fn.length > 33 ? fn.substring(0, 33) + "..." : fn;
            deleteSVG =
            ...
            docElement.innerHTML = `
               <div class="flex align-center gap-4 bg-[#f8fafb] px-4 py-3">
                    <div class="flex flex-1 flex-col justify-center">
                        <p class="text-[#0e141b] text-base font-medium leading-normal" title="${fn}">${filename}</p>
                    ...
                    <svg class="delete-button" data-filename="${fn}" style="width:20px;" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="size-6">
