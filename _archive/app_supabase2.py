...
@tool
def word_count(context: str, word: str) -> int:
    """counts the occurrences of a word in a text

    Args:
        context: text to count words in
        word: word to count
    """
    return context.lower().split().count(word.lower())

@tool
def query_document(context: str, query: str) -> str:
    """
    Queries a document for a given query.
    Args:
        context: The text of the document to query.
        query: The query to search for in the document.
    Returns:
        The answer to the query based on the document.
    """
    # Use the SupabaseVectorStore to retrieve relevant documents
    ...
    template = """You are an AI assistant for a document chatbot. Your primary function is to answer questions based on the content of various uploaded documents, including PDFs, Word documents, Excel spreadsheets, and text files. These documents have been processed, chunked, and stored in a Supabase database.
            ...
            - If the user wants to know how many times a word appears in a document, use the word_count tool to count the occurrences.
...
def query_document():
    ...
    try:
        ...
        tools = [word_count, query_document]

        llm_with_tools = llm.bind_tools(tools)
        tool_mapping = {"word_count": word_count, "query_document": query_document}
